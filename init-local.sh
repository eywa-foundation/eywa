...
# build the eywa chain with Rollkit
ignite chain build

# reset any existing genesis/chain data
eywad tendermint unsafe-reset-all

# initialize the validator with the chain ID you set
eywad init $VALIDATOR_NAME --chain-id $CHAIN_ID

# add keys for key 1 and key 2 to keyring-backend test
eywad keys add $KEY_NAME --keyring-backend test
eywad keys add $KEY_2_NAME --keyring-backend test

# add these as genesis accounts
eywad add-genesis-account $KEY_NAME $TOKEN_AMOUNT --keyring-backend test
eywad add-genesis-account $KEY_2_NAME $TOKEN_AMOUNT --keyring-backend test

# set the staking amounts in the genesis transaction
eywad gentx $KEY_NAME $STAKING_AMOUNT --chain-id $CHAIN_ID --keyring-backend test

# collect genesis transactions
eywad collect-gentxs
...
# eywad start --rollkit.aggregator true --rollkit.da_layer celestia --rollkit.da_config='{"base_url":"http://localhost:26658","timeout":60000000000,"fee":600000,"gas_limit":6000000,"auth_token":"'$AUTH_TOKEN'"}' --rollkit.namespace_id $NAMESPACE_ID --rollkit.da_start_height $DA_BLOCK_HEIGHT --rollkit.lazy_aggregator
